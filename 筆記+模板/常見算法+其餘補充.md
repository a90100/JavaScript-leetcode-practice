...
### Sliding Window 的正確性通常建立在「元素非負數」的假設。

Sliding Window 常用於**尋找符合某條件的子區間。**假設一個陣列中，元素有可能負數，則碰到求特定區間總合的情況時，

會因為新加入 window 的元素而導致 window 總和變大或變小，兩種可能性使得無法判斷當前 window 是不是最優解。

反之，若元素都是非負數，則新加入 window 的元素只會讓 window 總和變大，只有一種可能，具有單調性，那就只要確保當前總和是否符合條件即可。

參考 209. Minimum Size Subarray Sum 和 862. Shortest Subarray with Sum at Least K 的差別。

> 兩個都是找 >= k 的最短子陣列長，但是 209 陣列都是正數，862 陣列有負數
...
[排序演算法 維基百科](https://zh.wikipedia.org/wiki/%E6%8E%92%E5%BA%8F%E7%AE%97%E6%B3%95)
...
function bucketSort(nums) {
  ...
  // 根據數據自動計算 bucketSize
  const bucketSize = Math.ceil((max - min) / (nums.length - 1)) || 1;
  ...
  for (const bucket of buckets) {
    bucket.sort((a, b) => a - b); // 可替換為其他排序方法
    sorted.push(...bucket);
  }
...
console.log(bucketSort(nums)); // [3, 9, 21, 25, 29, 37, 43, 49]
```
